...
def get_arguments():
    ...
    parser.add_argument("--lr_decay", default="exp", help="Learning rate decay function, applied through lr scheduler. Accepted values: None, 'exp'")
    parser.add_argument("--lr_decay_rate", default=0.1, type=float, help="Learning rate decay rate, used if lr_decay = 'exp'")
    parser.add_argument("--lr_decay_step", default=(128*128*128/64)*1.0, type=float, help="Learning rate decay step, used if lr_decay = 'exp'")
    parser.add_argument("--lr_scheduler_call_frequency", default='batch', help="Learning rate scheduler call frequency, accepted values: None, 'epoch', 'batch'")
    # Early Stopping
    parser.add_argument("--early_stopping_min_delta", default=1e-2, type=float, help="Early stopping minimum (absolute) change in the monitored quantity to qualify as an improvement")
    ...
    # Loss & Metrics
    parser.add_argument("--loss", default="MSE", type=str, help="Loss function name, accepted values = ['MSE','RSE','Supervised_PINNS']")
    ...
    parser.add_argument("--regularizer_factor", default=0.1, type=float, help="Regularizer factor")
    # Training & Validation
